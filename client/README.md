...
the response returns the effective group pool. Placement findings are advisory
unless the owner group has active group-local pending pressure: in that case
both outside-owned-pool and borrowed placements are reported as violations.
Unknown owner-group pressure remains a warning with unknown evidence. Using
another group's node does not waive quota, and Pending workloads do not
receive placement findings. Group rows also expose their `pending_pressure`
state and threshold evidence.
...
`selected_groups`, or `outside_selected_groups`; the latter two reuse repeated
`--group` filters. `--placement-scope` accepts `any`, `owned_only`,
`borrowed_only`, `mixed`, or `includes_borrowed`. These filters use the pinned
snapshot's effective public node ownership. If node allocation is disabled,
the server reports the effective scope as unrestricted.

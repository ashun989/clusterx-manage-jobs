...
function RulesPage({ response, snapshot, error }: { response: PolicyResponse | null; snapshot: Snapshot; error: string }) {
  ...
  const includeHostnames = (nodes: string[]) => nodes.map((nodeName) => {
    const node = snapshot.nodes.find((item) => item.node === nodeName);
    return node ? nodePrimaryLabel(node) : nodeName;
  }).join(",");
  ...
  return <div className="rules-page">
    ...
    <section className="rules-section"><h3>分组 Quota</h3><div className="policy-groups"><table><thead><tr><th>Group ID</th><th>GPU quota</th><th>CPU quota</th><th>内存 quota GiB</th><th>成员数</th><th>有效节点</th><th>节点 GPU</th></tr></thead><tbody>{Object.entries(policy.groups).map(([group, config]) => { const nodes = allocationEnabled ? (effectiveAssignments[group] ?? []) : []; return <tr key={group}><td>{group}</td><td>{config.gpu_quota === "remainder" ? `remainder（当前 ${quota(snapshot.capacity.default_gpu_quota)}）` : quota(config.gpu_quota)}</td><td>{quota(config.cpu_quota)}</td><td>{quota(config.memory_quota_gib)}</td><td>{number(config.member_count)}</td><td>{allocationEnabled ? includeHostnames(nodes) || "无" : "全部 queue 节点可用"}</td><td>{allocationEnabled ? number(nodeGpuCapacity(nodes)) : "—"}</td></tr>; })}</tbody></table></div></section>

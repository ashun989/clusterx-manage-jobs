...
读写访问模式；`PV_AOSS` 会先在页面提供的 KMS 凭据下拉框中按 Bucket 名称做
大小写敏感的完全匹配。存在唯一同名凭据时直接选择该凭据；没有同名凭据时切换为
手动模式，并从 `metadata.items` 读取 `access_key` 与 `secret_key` 填写。同名凭据
不唯一时不会猜测，而是回退手填并提示复核。可提前在
[密钥管理 KMS](https://console.d.pjlab.org.cn/cn-pj-03/keypair/kms/credential)
保存以 Bucket 命名的 AK/SK 凭据。名称、实例规格、共享内存、WebIDE、SSH、优先级
继续使用页面当前值；填充结果会提醒用户手动调整实例规格，并确认共享内存、WebIDE、
SSH 访问和优先级后再提交。
...
仅在没有唯一同名 KMS 凭据时写入目标页面，不写入日志或扩展存储。工作空间、队列
或 RDMA 不匹配时会在镜像和对象存储凭据写入前停止。单个挂载失败不会回滚已成功
项目，弹窗会逐项说明使用了 KMS 还是手填，用户应在提交前人工复核。
